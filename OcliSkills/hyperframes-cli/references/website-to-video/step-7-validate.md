...
### 🍎 Minis (iOS) Alternative — browser_use snapshot

`npx hyperframes snapshot` requires Puppeteer. Use `minis-browser-use` with `set_viewport` to get accurate snapshots at the correct composition resolution:
...
# 3. Wait for runtime
minis-browser-use execute_js --script 'return !!window.__hf'

# 4. For each beat midpoint T:
minis-browser-use execute_js --script 'window.__hf.seek(<T>)'
minis-browser-use screenshot   # saved to /var/minis/browser/screenshot_*.jpg

# 5. Reset viewport when done
minis-browser-use set_viewport --reset
```
